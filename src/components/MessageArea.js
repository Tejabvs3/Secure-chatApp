...
export default function MessageArea() {
    const user  = JSON.parse(localStorage.getItem('user'));
       ...
       // const user = useSelector((state) => state.user);
        ...
        function handleSubmit(e) {
            ...
            console.log(roomId, encryptedMessage, user, time, todayDate)
            console.log(socket);
  ...
  return (
    ...
    <div className="messages-output" style={{height:"75vh",border:"1px solid lightgray",overflowY:"scroll",marginBottom:"20px",'&::WebkitScrollbar':{width:"thin"}}}>
                {localStorage.getItem('user') && !privateMemberMsg?._id && <div className="alert alert-success">You are in the {currentRoom} room</div>}
